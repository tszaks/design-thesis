...
Four documents live in this repo. This README is the one I wrote for people. [`LLM.md`](LLM.md) is the same law rewritten for machines: paste it into an AI agent at the start of a session and it will build like I do. [`APPLE_NATIVE.md`](APPLE_NATIVE.md) is the field guide: a catalog of Apple-native SwiftUI components (the drawers, the collapsing bars, the glass, the haptics, the hundred quirks you have to know exist before you can use them), every entry answering EXISTS, USE WHEN, and BUILD, with verified availability floors from iOS 13 to 26, fallbacks for older systems, and the gotchas that only show up in production. And [`APPLE_WEB.md`](APPLE_WEB.md) is the web sibling: how Apple builds apple.com and icloud.com without a line of Swift (it's Svelte, React, Vue, and an in-house system), and the five copy-able ingredients that make those sites feel native anyway, from the exact glass-nav CSS to the 17px type scale, each with browser floors and fallbacks. For iOS work, paste LLM.md and APPLE_NATIVE.md together; for web work, LLM.md and APPLE_WEB.md. Steal any of it. That's why it's public.
